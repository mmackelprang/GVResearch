...
- SIP messages (INVITE/ACK/BYE with SDP bodies) travel via **WebSocket** to `216.239.36.145:443` with subprotocol `"sip"`
- SIP domain: `web.c.pbx.voice.sip.google.com`
- SIP identity: encoded token from `sipregisterinfo/get` as SIP URI username
- Uses 100rel (PRACK), session timers (90s), Record-Route
- Google-specific headers: `X-Google-Client-Info`, `P-Preferred-Identity`, `X-GV-PlaceCallContext`
- The signaler BrowserChannel is only for post-call notifications (thread updates), NOT SDP exchange
- Full SIP capture with 14 frames: `captures/iaet-exports/gv-websocket-sip-capture.json`
- See `docs/investigations/gv-sdp-envelope-decoded.md` for complete architecture
...
4. **`SipCallTransport`** — `ICallTransport` using SIPSorcery SIP-over-WebSocket to `216.239.36.145:443`. Connect with `"sip"` subprotocol, REGISTER with Bearer auth from `sipregisterinfo/get`, INVITE with SDP. Full call flow captured in `gv-websocket-sip-capture.json`.
